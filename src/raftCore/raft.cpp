...
void Raft::doElection()
{
    ...
    if (m_status != Leader)
    {
        ...
        for (int i = 0; i < m_peers.size(); i++)
        {
            ...
            // 使用匿名函数执行避免其拿到锁
            m_iom->scheduler([this, i, requestVoteArgs, requestVoteReply, votedNum]()
...
void Raft::doHeartBeat()
{
    ...
    if (m_status == Leader)
    {
        ...
        for (int i = 0; i < m_peers.size(); i++)
        {
            ...
            m_iom->scheduler([this, i, appendEntriesArgs, appendEntriesReply, appendNums]()
    ...
    {
        std::cout << "node[" << m_me << "] not leeader,no needer send heart" << std::endl;
    }
}
...
void Raft::electionTimeOutTicker()
{
    ...
    while (true)
    {
        ...
        if (suitableSleepTime.count() > 1)
        {
            ...
            // std::cout << "node[" << m_me << "] begin eletion usleep" << std::endl;
            usleep(suitableSleepTime.count() / 1000);
            //sleep(4);
            // std::cout << "node[" << m_me << "] end eletion usleep" << std::endl;
        }

        if ((m_lastResetElectionTime - nowTime).count() > 0)
        {
            // 说明睡眠的这段时间有重置定时器，那么就没有超时，再次睡眠
            std::cout << "timer refresh while sleep" << std::endl;
            continue;
        }
        doElection();
    }
...
void Raft::leaderHearBeatTicker()
{
    while (true)
    {
        ...
        // std::this_thread::sleep_for(suitableSleepTime);
        // 睡眠，并退出当前协程，直到时间到达
        std::cout << "node[" << m_me << "] begin heartbeat usleep" << std::endl;
        usleep(suitableSleepTime.count() / 1000);
        //sleep(2);
        std::cout << "node[" << m_me << "] end heartbeat usleep" << std::endl;
        if ((m_lastResetHearBeatTime - nowTime).count() > 0)
        {
            // 说明睡眠的这段时间有重置定时器，那么就没有超时，再次睡眠
            std::cout << "heart beat weak but continue" << std::endl;
            continue;
        }
        doHeartBeat();
    }
        ...
        *appendNums = *appendNums + 1;
        ...
        if (*appendNums >= 1 + m_peers.size() / 2)
        {
            ...
            *appendNums = 0;
            ...
            {
                DPrintf(
                    "---------------------------tmp------------------------- 當前term有log成功提交，更新leader的m_commitIndex from{%d} to{%d}",
                ...
                std::shared_ptr<LockQueue<ApplyMsg>> applyCh)
{
    ...
    // 初始化fiber管理器 (默认不适用call_thread执行调度任务)
    m_iom = std::make_shared<monsoon::IOManager>(THREAD_NUM, USE_CALLER_THREAD);
    ...
    std::cout << "[" << m_me << "]:raft init success" << std::endl;
}
